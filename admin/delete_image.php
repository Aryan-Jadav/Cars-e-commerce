<?php
// Get the image filename and type from the AJAX request
$imageName = $_POST['image'];
$imageType = $_POST['type'];

// Define the path to the image file
$imagePath = "Add Car/" . $imageName;
...
if ($imageType === 'single') {
    ...
    $sql = "UPDATE categories SET car_image = '' WHERE car_image = '$imageName'";
    ...
    // Update the car_image field in the database with an empty string or a default image path
    $sql = "UPDATE categories SET car_image = '' WHERE car_image = '$imageName'";
    mysqli_query($conn, $sql);
} else if ($imageType === 'multiple') {
    $sql = "SELECT car_detail_img FROM categories";
    ...
    while ($row = mysqli_fetch_assoc($result)) {
        ...
        if ($key !== false) {
            ...
            $sql_update = "UPDATE categories SET car_detail_img = '$new_images' WHERE car_detail_img LIKE '%$imageName%'";
